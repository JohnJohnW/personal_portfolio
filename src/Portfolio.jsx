...
import {
  Github, Linkedin, Mail, ArrowRight, Rocket, Sparkles, ChevronRight,
  Folder, Cpu, Wand2, Download, Phone, Globe, PlaySquare
} from "lucide-react";

/* ================== Always-Dark Portfolio ================== */
/* This component forces the site into dark mode on mount by adding
   `class="dark"` to <html>. There is no theme toggle. */
...
function LiquidBlobs() {
  return (
    <div className="fixed -z-10 inset-0 overflow-hidden">
      <div className="absolute -top-24 -left-16 h-[40rem] w-[40rem] rounded-full bg-gradient-to-br from-fuchsia-500/25 via-indigo-500/20 to-blue-500/20 blur-3xl opacity-70 mix-blend-screen animate-pulse" />
      <div className="absolute -bottom-24 -right-16 h-[38rem] w-[38rem] rounded-full bg-gradient-to-tr from-cyan-400/20 via-sky-500/20 to-indigo-600/20 blur-3xl opacity-70 mix-blend-screen animate-[pulse_9s_ease-in-out_infinite]" />
    </div>
  );
}
...
function Dock({ items }) {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <div className="backdrop-blur-xl bg-black/30 border border-white/10 shadow-2xl rounded-2xl px-3 py-2 flex gap-2">
        {items.map((it, i) => (
          <button
            key={i}
    ...
    <section id={id} className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      {subtitle && (
        <motion.p
          ...
          className="mt-2 text-sm/6 text-neutral-300"
        >
          {subtitle}
        </motion.p>
      )}
...
export default function Portfolio() {
  // Force dark mode on mount (no toggle)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(1200px_800px_at_20%_-10%,rgba(59,130,246,.18),transparent),radial-gradient(1200px_800px_at_80%_110%,rgba(236,72,153,.14),transparent)] text-neutral-100 selection:bg-indigo-500/30">
      <LiquidBlobs />
      <Spotlight />
          ...
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <nav className="hidden sm:flex items-center gap-1">
              {[
                ...
                <button
                  ...
                  className={cn(
                    "px-3 py-2 rounded-xl text-sm hover:bg-white/10 transition",
            ...
            <div className="flex items-center gap-2">
              <a
                ...
                className="hidden sm:inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-3 py-2 text-sm hover:bg-white/20 transition"
              >
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <motion.p
              ...
              className="mt-4 text-base/7 text-neutral-300"
            >
              {DATA.blurb}
            </motion.p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/10 px-4 py-2 backdrop-blur-xl hover:bg-white/20 transition"
              >
            ...
            <div className="mt-6 flex gap-3">
              {DATA.socials.map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer" className="group">
                  <div className="h-10 w-10 grid place-items-center rounded-xl border border-white/10 bg-white/10 backdrop-blur-xl group-hover:bg-white/20 transition">
              ...
              >
                ...
                <div className="mt-5 grid grid-cols-3 gap-3">
                  {["Python", "TypeScript", "React", "FastAPI", "n8n", "OpenAI"].map((sk) => (
                    <div
                      key={sk}
                      className="rounded-2xl border border-white/10 bg-white/10 p-3 text-center text-sm"
                    >
                      {sk}
                    </div>
                  ))}
          ...
          {DATA.projects.map((p, i) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {p.stack.map((s) => (
                  <span key={s} className="text-xs rounded-lg border border-white/10 bg-white/10 px-2 py-1">
                    {s}
                  </span>
                ))}
              </div>
      ...
      <Section id="videos" title="Videos" icon={PlaySquare} subtitle="Latest uploads from my YouTube channel.">
        ...
        <div className="mt-6 flex justify-end">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-sm inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-3 py-2 backdrop-blur-xl hover:bg-white/20"
          >
          ...
          <ul className="space-y-6">
            {[
              ...
              },
              {
                ...
                when: "2024 — Present",
                bullets: ["Organised legal-tech hackathons and AI workshops", "Partnered with firms for student projects"],
              },
            ].map((e, i) => (
              <motion.li
                key={i}
        ...
        >
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-6 items-center">
            ...
            <div className="flex md:justify-end gap-3">
              ...
              <a
                href={DATA.resume}
                className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 py-2 backdrop-blur-xl hover:bg-white/20"
              >
          ...
          <div className="grid gap-3">
            <label className="text-sm">
              ...
              <input
                name="name"
                required
                className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </label>
            <label className="text-sm">
              Email
            ...
            <label className="text-sm">
              ...
              <select
                name="budget"
                className="mt-1 w-full rounded-xl bg-white/10 border border-white/10 px-3 py-2"
              >
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[60] rounded-2xl border border-white/10 bg-black/70 backdrop-blur-xl px-4 py-2 shadow-xl"
          >
            {toast}
          </motion.div>
        )}
